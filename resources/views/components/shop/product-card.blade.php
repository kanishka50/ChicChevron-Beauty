...
<div class="group relative bg-white rounded-2xl shadow-sm hover:shadow-xl transition-all duration-300 overflow-hidden">
    ...
    <div class="absolute top-3 left-3 z-10 flex gap-2">
        ...
        @if($product->discount_price && $product->discount_price < $product->selling_price)
            ...
            @endphp
            <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-bold bg-gradient-to-r from-red-500 to-rose-600 text-white shadow-md">
                -{{ $discountPercent }}%
            </span>
        @endif
    </div>
    ...
    <div class="relative aspect-square overflow-hidden bg-gray-50">
        ...
        <div class="absolute inset-0 bg-gradient-to-t from-black/10 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 hidden sm:block"></div>
        ...
        <div class="absolute top-3 right-3 flex flex-col gap-2">
            ...
            <button type="button"
                    ...
                    class="p-2 sm:p-2.5 bg-white/90 backdrop-blur-sm rounded-full shadow-lg hover:bg-white transition-all duration-200 transform hover:scale-110"
                    data-product-id="{{ $product->id }}">
                <svg class="w-4 h-4 sm:w-5 sm:h-5 {{ $isInWishlist ? 'text-primary-600 fill-current' : 'text-gray-600' }}" 
            ...
            <button onclick="quickView({{ $product->id }})" 
                    class="hidden sm:flex p-2.5 bg-white/90 backdrop-blur-sm rounded-full shadow-lg hover:bg-white transition-all duration-200 transform hover:scale-110 opacity-0 group-hover:opacity-100 items-center justify-center">
    ...
    <div class="p-3 sm:p-4 md:p-5 space-y-2 sm:space-y-3">
        ...
        <!-- Product Name -->
        <h3 class="font-semibold text-sm sm:text-base text-gray-900 overflow-hidden group-hover:text-primary-600 transition-colors min-h-[2.5rem] sm:min-h-[3rem]"
        ...
        @if($product->reviews_count > 0)
            <div class="flex items-center gap-1">
                <div class="flex text-yellow-400">
                    @for($i = 1; $i <= 5; $i++)
                        <svg class="w-3 h-3 sm:w-4 sm:h-4 {{ $i <= $product->average_rating ? 'fill-current' : 'fill-gray-200' }}" viewBox="0 0 20 20">
        ...
        <!-- Variants Preview - Simplified for Mobile -->
                ...
                @if($colorVariants->isNotEmpty())
                    <div class="flex items-center gap-1">
                        @foreach($colorVariants as $variant)
                            ...
                            @endphp
                            <div class="w-5 h-5 sm:w-6 sm:h-6 rounded-full border-2 border-white shadow-md transition-transform hover:scale-110" 
                                 style="background-color: {{ $colorCode }}"
                                 title="{{ $variant->variant_value }}"></div>
                        @endforeach
                        @if($product->variants->where('variant_type', 'color')->count() > 4)
                            <span class="text-xs text-gray-500 ml-1">+{{ $product->variants->where('variant_type', 'color')->count() - 4 }}</span>
                        @endif
                    </div>
                @endif
        ...
        <div class="pt-2">
            @if($product->hasStock())
                @if($product->has_variants)
                    <a href="{{ route('products.show', $product->slug) }}" 
                       class="w-full flex items-center justify-center gap-2 bg-gray-100 text-gray-700 py-2.5 sm:py-3 px-4 rounded-xl hover:bg-gray-200 transition-all font-medium text-sm sm:text-base group min-h-[44px]">
                ...
                @else
                    <button onclick="addToCart({{ $product->id }}, null, 1)" 
                            class="w-full flex items-center justify-center gap-2 bg-gradient-to-r from-primary-600 to-primary-700 text-white py-2.5 sm:py-3 px-4 rounded-xl hover:from-primary-700 hover:to-primary-800 transition-all font-medium text-sm sm:text-base shadow-md hover:shadow-lg min-h-[44px] active:scale-[0.98] transform"
                            data-product-id="{{ $product->id }}">
            ...
            @else
                <button disabled 
                        class="w-full bg-gray-200 text-gray-400 py-2.5 sm:py-3 px-4 rounded-xl font-medium cursor-not-allowed text-sm sm:text-base min-h-[44px]">
                    Out of Stock
                </button>
            @endif
        </div>
