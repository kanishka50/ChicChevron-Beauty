    ...
    <div class="container-responsive">
        <div class="max-w-4xl mx-auto">
            ...
            <div class="text-center mb-8">
                ...
                <div class="mb-6">
                    <div class="w-16 h-16 md:w-20 md:h-20 lg:w-24 lg:h-24 mx-auto relative">
                        ...
                        <div class="relative bg-gradient-to-br from-green-400 to-green-600 rounded-full w-20 h-20 md:w-24 md:h-24 flex items-center justify-center shadow-lg">
                            <svg class="w-8 h-8 md:w-10 md:h-10 lg:w-12 lg:h-12 text-white" fill="currentColor" viewBox="0 0 20 20">
                ...
                <h1 class="text-2xl md:text-3xl lg:text-4xl font-bold text-gray-900 mb-3">Order Confirmed!</h1>
                <p class="text-base md:text-lg text-gray-600 mb-2">Thank you for your order</p>
            ...
            <div class="bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden mb-6">
                ...
                <div class="bg-gradient-to-r from-primary-600 to-primary-700 px-6 md:px-8 py-6 text-white">
                    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                        <div>
                            <h2 class="text-lg md:text-xl font-semibold mb-1">Order Details</h2>
                            <p class="text-white/90 text-xs md:text-sm">Order placed on {{ $order->created_at->format('F d, Y \a\t g:i A') }}</p>
                        </div>
                        <div class="bg-white/20 backdrop-blur rounded-xl px-4 py-3">
                            <p class="text-xs text-white/80 mb-0.5">Order Number</p>
                            <p class="text-base md:text-lg font-bold">{{ $order->order_number }}</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="p-6 md:p-8 border-b border-gray-100">
                    <div class="flex items-center justify-between max-w-lg mx-auto">
                        ...
                        <div class="flex flex-col items-center">
                            <div class="w-8 h-8 md:w-10 md:h-10 bg-green-100 rounded-full flex items-center justify-center mb-2">
                        ...
                        <!-- Processing -->
                        <div class="flex flex-col items-center">
                            <div class="w-8 h-8 md:w-10 md:h-10 bg-gray-100 rounded-full flex items-center justify-center mb-2">
                ...
                <div class="p-6 md:p-8 grid grid-cols-1 md:grid-cols-2 gap-6 border-b border-gray-100">
                    ...
                    <div>
                        <h3 class="font-semibold text-gray-900 mb-3 flex items-center gap-2 text-sm md:text-base">
                ...
                <div class="p-6 md:p-8 flex flex-col md:flex-row md:items-center md:justify-between gap-4 border-b border-gray-100">
                    <div>
                        <h3 class="font-semibold text-gray-900 mb-3 flex items-center gap-2 text-sm md:text-base">Payment Method</h3>
                        <p class="text-xs md:text-sm text-gray-600">{{ ucfirst(str_replace('_', ' ', $order->payment_method)) }}</p>
                    </div>
                    <div class="text-left md:text-right">
                        <p class="text-xs md:text-sm text-gray-500 mb-1">Order Total</p>
                        <p class="text-xl md:text-2xl font-bold text-primary-600">Rs. {{ number_format($order->total_amount, 2) }}</p>
                    </div>
                </div>

                <!-- Order Items -->
                <div class="p-6 md:p-8">
                    <h3 class="font-semibold text-gray-900 mb-4 text-sm md:text-base">Order Items ({{ $order->items->count() }})</h3>
                        ...
                        @foreach($order->items as $item)
                            <div class="flex items-start gap-4 pb-4 border-b border-gray-100 last:border-0 last:pb-0">
                                <img src="{{ $item->product->main_image ? asset('storage/' . $item->product->main_image) : '/placeholder.jpg' }}" 
                                     ...
                                     class="w-14 h-14 md:w-16 md:h-16 lg:w-20 lg:h-20 object-cover rounded-lg flex-shrink-0">
                                <div class="flex-1 min-w-0">
                                    <h4 class="font-medium text-gray-900 text-xs md:text-sm">{{ $item->product_name }}</h4>
                                    {{-- @if($item->variant_details)
                                        <p class="text-xs md:text-sm text-gray-500 mt-0.5">{{ $item->variant_details }}</p>
                                    @endif --}}
                                    <p class="text-xs text-gray-400 mt-1">SKU: {{ $item->product_sku }}</p>
                                </div>
                                <div class="text-right flex-shrink-0">
                                    <p class="text-xs md:text-sm text-gray-600">Qty: {{ $item->quantity }}</p>
                                    <p class="font-medium text-gray-900 text-xs md:text-sm">Rs. {{ number_format($item->total_price, 2) }}</p>
                                </div>
                            </div>
                        @endforeach
                    </div>
                    ...
                    <div class="mt-6 pt-6 border-t border-gray-200 space-y-2">
                        ...
                        <div class="lex justify-between font-semibold text-base md:text-lg pt-2 border-t">
                            <span>Total</span>
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 md:p-6 lg:p-8">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <a href="{{ route('user.orders.show', $order) }}" 
                       class="inline-flex items-center justify-center gap-2 bg-gradient-to-r from-primary-600 to-primary-700 text-white py-2.5 px-4 md:py-3 md:px-6  rounded-lg hover:from-primary-700 hover:to-primary-800 font-medium transform hover:scale-[1.02] transition-all duration-200 shadow-lg">
                    ...
                    <a href="{{ route('products.index') }}" 
                       class="inline-flex items-center justify-center gap-2 bg-gray-100 text-gray-800 py-2.5 px-4 md:py-3 md:px-6  rounded-lg hover:bg-gray-200 font-medium transition-all duration-200">
                        <svg class="w-4 h-4 md:w-5 md:h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="mt-6 pt-6 border-t border-gray-200">
                    <div class="bg-blue-50 rounded-xl p-4">
                        <div class="flex items-start gap-3">
                            <svg class="w-4 h-4 md:w-5 md:h-5 text-blue-600 flex-shrink-0 mt-0.5" fill="currentColor" viewBox="0 0 20 20">
